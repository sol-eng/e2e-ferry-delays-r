...
Value of `CONDITION_OVERRIDE` is `r Sys.getenv("CONDITION_OVERRIDE")`
...
## Validate raw production data schema
...
To simply this document, cleaning steps have been wrapped in a function in the `ferryfairy` package. 
...
if (exists("main_agent")) {main_agent}
```
...
## Remove failed records from data set
...
## Write cleaned and validated data to database
...
if (stop_alert != TRUE & !Sys.getenv("CONDITION_OVERRIDE") %in% conditions) {
  ...
  my_df_name <-"modeldata_validated"
...
if (condition == "WARN") {
  ...
  # threshold will be NULL if condition override is set to trigger this email.
  if(is.null(threshold)) threshold <- rep(0, length(step))
